...
const CATEGORY_NAMES: Record<string, string> = {
  photo: "УГАДАЙ ПО ФОТО",
  music: "УГАДАЙ ПО МУЗЫКЕ",
  video: "УГАДАЙ ПО ВИДЕО"
};
...
export function LatestQuestionsTable() {
  ...
  const loadQuestions = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await fetch(`/api/questions?page=${page}&limit=10&sort=createdAt:desc`, { cache: "no-store" });
  ...
  const firstShown = total === 0 ? 0 : (page - 1) * 10 + 1;
  const lastShown = Math.min(page * 10, total);

  return (
    <section className="mt-8">
      <div className="mb-4 flex flex-col justify-between gap-2 md:flex-row md:items-end">
        <div>
          <h2 className="text-3xl font-extrabold tracking-[-0.03em] text-foreground">Последние добавленные вопросы</h2>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-[920px] w-full border-collapse text-left">
            ...
            <tbody className="divide-y divide-black/[0.06]">
              {loading && (
                <tr>
                  <td colSpan={6} className="px-5 py-10 text-center text-muted">Загружаем вопросы...</td>
                </tr>
              )}
              {!loading && error && (
                <tr>
                  <td colSpan={6} className="px-5 py-10 text-center text-danger">{error}</td>
                </tr>
              )}
              {!loading && !error && data?.items.length === 0 && (
                <tr>
                  <td colSpan={6} className="px-5 py-10 text-center text-muted">Пока нет добавленных вопросов</td>
                </tr>
              )}
              {!loading && !error && data?.items.map((question) => (
                <tr key={question.id} className="transition hover:bg-primary/[0.04]">
                  <td className="max-w-[160px] truncate px-5 py-4 font-mono text-xs text-muted" title={question.id}>{question.id}</td>
                  ...
                  <td className="px-5 py-4 text-muted">{CATEGORY_NAMES[question.topicCard.quizType.type] ?? "Неизвестно"}</td>
                  <td className="px-5 py-4">
